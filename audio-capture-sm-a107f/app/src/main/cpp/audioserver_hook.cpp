...
static std::unordered_map<pid_t, std::string> gPidToProcess;
...
static pid_t readCreatorPid(void* thisPtr) {
    return *reinterpret_cast<pid_t*>(
        reinterpret_cast<uint8_t*>(thisPtr) + CREATOR_PID_OFFSET);
}
...
static size_t calculateFrameSize(void* thisPtr) {
    uint32_t isOut = *reinterpret_cast<uint8_t*>(
...
static const char* getTargetPackage(pid_t pid) {
    auto it = gPidToProcess.find(pid);
    if (it == gPidToProcess.end()) {
        char path[64];
        snprintf(path, sizeof(path), "/proc/%d/cmdline", pid);
        FILE* fp = fopen(path, "rb");
        if (!fp) {
            LOGD("Cannot read process name for PID %d", pid);
            return nullptr;
        }
        char processName[256] = {};
        const size_t count = fread(processName, 1, sizeof(processName) - 1, fp);
        fclose(fp);
        if (count == 0) return nullptr;
        it = gPidToProcess.emplace(pid, processName).first;
    }
    ...
    for (auto target : kTargetPackages) {
        ...
        if (processName.compare(0, targetLength, target) == 0 &&
                (processName.size() == targetLength || processName[targetLength] == ':')) {
            LOGD("PID %d -> %s [MATCH], recording", pid, processName.c_str());
            return target;
        }
    }

    LOGD("PID %d -> %s [NOT TARGET], skipping", pid, processName.c_str());
    return nullptr;
}
...
//    LOGD("RecordTrack::getNextBuffer called");
    ...
    pid_t creatorPid = readCreatorPid(thisPtr);
    uint32_t sampleRate = readSampleRate(thisPtr);

    const char* pkg = getTargetPackage(creatorPid);
    if (!pkg) {
        LOGD("PID %d is not a target app, skipping", creatorPid);
        return ret;
    }
    ...
    size_t bufferSize = buffer->frameCount * 0.5; // Mic frameSize 0.5

    FILE* file = openTrackFile(thisPtr, creatorPid, pkg, sampleRate, "uplink");
...
//    LOGD("Track::getNextBuffer called");
    ...
    FILE* file = openTrackFile(thisPtr, creatorPid, pkg, sampleRate, "downlink");
    if (file && bufferSize > 0) {
        size_t written = fwrite(buffer->raw, 1, bufferSize, file);
...
extern "C" void init_hooks() {
    ...
    if (DobbyHook(addr, (void*)hook_AudioFlinger_setMode,
        ...
        LOGE("Failed to hook AudioFlinger::setMode");
    ...
    if (DobbyHook(addr,
        ...
        LOGD("Hooked RecordTrack::getNextBuffer at %p", addr);
    } else {
        LOGE("Hook failed for RecordTrack::getNextBuffer at %p", addr);
    }
    ...
    if (DobbyHook(addr,
        ...
        LOGD("Hooked Track::getNextBuffer at %p", addr);
    } else {
        LOGE("Hook failed for Track::getNextBuffer at %p", addr);
    }
    ...
    if (DobbyHook(addr,
        ...
        LOGD("Hooked Track::stop at %p", addr);
    } else {
        LOGE("Hook failed for Track::stop at %p", addr);
    }
    ...
    if (DobbyHook(addr,
        ...
        LOGD("Hooked RecordTrack::stop at %p", addr);
    } else {
        LOGE("Hook failed for RecordTrack::stop at %p", addr);
    }


    LOGD("Audio hooks initialized");
