...
extern "C"
JNIEXPORT void JNICALL
Java_tn_amin_phantom_1mic_PhantomManager_nativeSetAiMode(JNIEnv *env, jobject thiz, jboolean ai_mode) {
    if (g_phantomBridge != nullptr) {
        g_phantomBridge->set_ai_mode(ai_mode == JNI_TRUE);
    }
}

extern "C"
JNIEXPORT jboolean JNICALL
Java_tn_amin_phantom_1mic_PhantomManager_nativeIsPlaybackFinished(JNIEnv *env, jobject thiz) {
    if (g_phantomBridge == nullptr) {
        return JNI_FALSE;
    }
    return g_phantomBridge->is_playback_finished() ? JNI_TRUE : JNI_FALSE;
}

extern "C"
JNIEXPORT void JNICALL
Java_tn_amin_phantom_1mic_PhantomManager_nativeResetAudioBuffer(JNIEnv *env, jobject thiz) {
    if (g_phantomBridge != nullptr) {
        g_phantomBridge->reset_audio_buffer();
    }
}
